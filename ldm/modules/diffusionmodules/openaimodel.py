    ...
    def forward(self, x, timesteps=None, context=None, y=None, features_adapter=None, append_to_context=None, **kwargs):
        ...
        for id, module in enumerate(self.input_blocks):
            ...
            if ((id+1)%3 == 0) and features_adapter is not None:
                ...
                mean_h, std_h = torch.mean(h, dim=(2, 3), keepdim=True), torch.std(h, dim=(1, 2, 3), keepdim=True)
                mean_control, std_control = torch.mean(controls, dim=(2, 3), keepdim=True), torch.std(controls, dim=(1, 2, 3), keepdim=True)
...
#--------------------------------------------------------------------------------------------------------------------
class UNetModel_DECODE(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def forward(self, x, timesteps=None, context=None, y=None, features_adapter=None, append_to_context=None, **kwargs):
        ...
        if features_adapter is not None:
            ...
            controls = (controls - mean_control) * (std_h / (std_control + 1e-12)) + mean_h
            # controls = nn.functional.adaptive_avg_pool2d(controls, h.shape[-2:])
            h = h + controls * 1.0     #TODO: 1.0           
            # h = h + features_adapter[adapter_idx]
            adapter_idx -= 1
        
        for idx,module in enumerate(self.output_blocks):
            if (idx in [3,6,9]) and features_adapter is not None and adapter_idx>=0:
                controls=features_adapter[adapter_idx]
                mean_h, std_h = torch.mean(h, dim=(1, 2, 3), keepdim=True), torch.std(h, dim=(1, 2, 3), keepdim=True)
                mean_control, std_control = torch.mean(controls, dim=(1, 2, 3), keepdim=True), torch.std(controls, dim=(1, 2, 3), keepdim=True)
                controls = (controls - mean_control) * (std_h / (std_control + 1e-12)) + mean_h
                h = torch.cat([h, hs.pop() + controls], dim=1)
                adapter_idx -= 1
            else:
                h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb, context)
