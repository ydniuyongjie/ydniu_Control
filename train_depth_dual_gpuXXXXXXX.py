...
def initialize_depth_estimator(weight_path="depth_weight/depth_anything_v2_vitl.pth", is_main_process=True):
    """
...
def calculate_depth_consistency_metrics(generated_depth, original_depth, generated_img=None, target_img=None):
    ...
    try:
        ...
        if metrics['score_ssim'] is not None:
            ...
            # 如果SSIM不可用，重新分配权重：Score_MAE(0.222) + Score_GradMAE(0.444) + Score_LPIPS(0.333)
            # 权重按原比例缩放：原权重0.1+0.2+0.35=0.65，缩放因子为1/0.65≈1.538
            composite_score = (0.222 * metrics['score_mae']) + (0.444 * metrics['score_grad_mae']) + (0.333 * metrics['score_lpips'])
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="gradient accumulation steps. Effective batch size = bsize * world_size * grad_accum_steps"
    )
    parser.add_argument(
        '--use_wandb',
        action='store_true',
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    # 初始化分布式训练
    opt.rank = opt.gpu
    dist.init_process_group(backend='nccl', init_method=opt.dist_url, world_size=ngpus_per_node, rank=opt.rank)
            ...
            if hasattr(opt, 'wandb_run_id') and opt.wandb_run_id:
                wandb.init(
                    ...
                    config={
                        ...
                        "effective_batch_size": opt.bsize * opt.world_size * opt.grad_accum_steps
                    }
                )
            else:
                wandb.init(
    ...
    # stable diffusion - 所有进程都需要加载SD模型用于训练损失计算
    ...
    if opt.rank == 0:
        ...
        with torch.no_grad():
            ...
            if opt.dpm_solver:
                ...
                sampler = DDIMSamplerNIU(model)  # 使用与train_depth_niu.py相同的采样器
    ...
    # depth encoder - 所有进程都需要
    model_ad = Adapter(cin=3 * 64, channels=[320, 640, 1280, 1280][:4], nums_rb=2, ksize=1, sk=True, use_conv=False).to(device)
    model_ad = torch.nn.parallel.DistributedDataParallel(model_ad, device_ids=[opt.gpu])

    # Control Injection Blocks - 所有进程都需要
    adapter_channels = [320, 640, 1280, 1280][:4]  # 与Adapter通道数对应
    control_injectors = torch.nn.parallel.DistributedDataParallel(
        nn.ModuleList(
    ...
    # 初始化深度估计器用于验证 - 只在主进程
    if opt.rank == 0:
        depth_estimator = initialize_depth_estimator("depth_weight/depth_anything_v2_vitl.pth", is_main_process=True)
    ...
    # 计算模型参数量 - 只在主进程显示
    model_ad_params = None
    control_injectors_params = None
    total_params = None
    ...
    if opt.rank == 0:
        ...
        # 获取原始模型（非DDP包装）来计算参数量
        model_ad_original = model_ad.module
        control_injectors_original = control_injectors.module

        # 计算各模型参数量
        model_ad_params = count_parameters(model_ad_original)
        model_ad_trainable_params = count_trainable_parameters(model_ad_original)

        control_injectors_params = count_parameters(control_injectors_original)
        control_injectors_trainable_params = count_trainable_parameters(control_injectors_original)
    ...
    # optimizer - 每个进程都有自己的optimizer
    trainable_params = itertools.chain(
        model_ad.parameters(),
        control_injectors.parameters()  # 添加control_injectors参数到优化器
    )
    optimizer = torch.optim.AdamW(trainable_params, lr=config['training']['lr'])

    # 计算总批次数 (需要在调度器创建前计算)
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / opt.world_size)

    # 使用带预热的余弦衰减学习率调度器（与单卡训练一致）
    class WarmupCosineAnnealingLR:
        ...
        Args:
            ...
            warmup_steps: 预热步数 (例如 2000)
            max_lr: 峰值学习率 (例如 2e-5)
            min_lr: 最小学习率 (例如 1e-7)
            warmup_start_lr: 预热起始学习率 (例如 1e-7)
            total_steps: 总训练步数
        """
        def __init__(self, optimizer, warmup_steps=2000, max_lr=2e-5, min_lr=1e-7,
                     warmup_start_lr=1e-7, total_steps=None):
        ...
        def step(self, current_step=None):
            ...
            if self.current_step <= self.warmup_steps:
                # 预热阶段：从warmup_start_lr线性增加到max_lr
                lr = self.warmup_start_lr + (self.max_lr - self.warmup_start_lr) * (self.current_step / self.warmup_steps)
            else:
                # 余弦衰减阶段
                if self.total_steps:
                    # 使用总步数计算余弦衰减
                    ...
                    # 如果没有总步数，使用持续余弦衰减
                    cosine_period = 10000  # 余弦衰减周期，可调整
                    progress = ((self.current_step - self.warmup_steps) % cosine_period) / cosine_period
                    lr = self.min_lr + (self.max_lr - self.min_lr) * 0.5 * (1 + math.cos(math.pi * progress))
    ...
    scheduler = WarmupCosineAnnealingLR(
        optimizer=optimizer,
        warmup_steps=2000,      # 前6000步预热
        max_lr=1e-5,            # 峰值学习率 2e-5
        min_lr=1e-7,            # 最小学习率 1e-7
        warmup_start_lr=1e-7,   # 预热起始学习率 1e-7
        total_steps=total_training_steps  # 总训练步数
    )

    # 【修复 Bug】添加"热身启动"修复代码
    for group in optimizer.param_groups:
        group['lr'] = scheduler.warmup_start_lr

    # 基于图像质量评估 - 只在主进程（早停机制已禁用，改为固定epochs训练）
    if opt.rank == 0:
        best_val_score = -float('inf')  # 越大越好（SSIM、PSNR）
        worst_lpips = float('inf')      # 越小越好（LPIPS）
        patience = 20  # 早停耐心值 (已禁用)
        patience_counter = 0  # 早停计数器 (已禁用)
        best_model_state = None  # 最佳模型状态
        best_control_injectors_state = None  # 最佳ControlInjectionBlock模型状态
        # early_stop = False  # 早停标志 (已禁用)

    # resume state - 只在主进程加载和保存状态
    resume_state, resume_ckpt, best_ckpt = None, None, None
    if opt.rank == 0:
    ...
    if opt.rank == 0:
        if resume_state is None or resume_ckpt is None:
            ...
            # 加载ControlInjectionBlock状态
            if hasattr(resume_state, 'get') and 'scheduler' in resume_state:
                scheduler.load_state_dict(resume_state['scheduler'])
                logger.info("Training has resumed.So Loaded scheduler state")

            # 尝试加载ControlInjectionBlock权重
            control_injectors_ckpt_path = os.path.join('experiments', opt.instance_name, 'models', f'model_control_injectors_{resume_state["iter"]:.0f}.pth')
            if os.path.exists(control_injectors_ckpt_path):
                control_injectors_ckpt = torch.load(control_injectors_ckpt_path, map_location=lambda storage, loc: storage.cuda(opt.gpu))
                ...
                logger.warning(f"ControlInjectionBlock checkpoint not found at {control_injectors_ckpt_path}, starting with random initialization")

        # 将参数信息记录到日志（在logger初始化后）
        if model_ad_params is not None:
            def params_to_millions(params):
                return params / 1_000_000
            ...
            logger.info("📊 模型参数量统计")
            logger.info("=" * 60)
            logger.info(f"Adapter (model_ad): {model_ad_params:,} ({params_to_millions(model_ad_params):.2f}M) 参数")
            logger.info(f"ControlInjectionBlock: {control_injectors_params:,} ({params_to_millions(control_injectors_params):.2f}M) 参数")
            ...
            if opt.use_wandb and wandb_available:
                wandb.config.update({
                    "model_ad_params": model_ad_params,
                    "model_ad_params_millions": params_to_millions(model_ad_params),
                    "control_injectors_params": control_injectors_params,
                    "control_injectors_params_millions": params_to_millions(control_injectors_params),
                    "total_params": total_params,
                    "total_params_millions": params_to_millions(total_params)
                })

    # copy the yml file to the experiment root - 只在主进程
    if opt.rank == 0:
    ...
    if opt.rank == 0:
        ...
        logger.info(f"  Num processes = {opt.world_size}")
        logger.info(f"  Process rank = {opt.rank}")
        logger.info(f"  Device = {device}")
        logger.info(f"  Num examples = {len(train_dataset)}")
        logger.info(f"  Num Epochs = {opt.epochs}")
        logger.info(f"  Batch size per device = {opt.bsize}")
        logger.info(f"  Total batch size (without accumulation) = {opt.bsize * opt.world_size}")
        logger.info(f"  Gradient accumulation steps = {opt.grad_accum_steps}")
        logger.info(f"  Effective batch size = {opt.bsize * opt.world_size * opt.grad_accum_steps}")
        ...
        logger.info(f"  Learning rate scheduler = WarmupCosineAnnealingLR (warmup=2000, max_lr=1e-5, min_lr=1e-7)")
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            # 梯度累积逻辑
            # 只在累积步骤开始时清零梯度
            if (batch_idx % opt.grad_accum_steps) == 0:
                optimizer.zero_grad()
                model.zero_grad()

            # 使用 no_sync() 优化梯度累积
            # 前面的累积步骤不进行梯度同步，只在最后一步同步
            is_last_step = (batch_idx + 1) % opt.grad_accum_steps == 0

            # 对于DDP包装的control_injectors，需要通过.module访问原始ModuleList
            control_injectors_module = control_injectors.module

            # 同时对两个DDP模型使用no_sync()优化
            if not is_last_step:
                with model_ad.no_sync(), control_injectors.no_sync():
                    features_adapter = model_ad(depth_data)
                    l_pixel, loss_dict = model(z, c=c, features_adapter=features_adapter, control_injectors=control_injectors_module)
                    # 缩放损失以支持梯度累积
                    l_pixel = l_pixel / opt.grad_accum_steps
                    l_pixel.backward()
            else:
                features_adapter = model_ad(depth_data)
                l_pixel, loss_dict = model(z, c=c, features_adapter=features_adapter, control_injectors=control_injectors_module)
            ...
            if is_last_step:
                ...
                optimizer.step()
                scheduler.step()  # 每次优化器步骤后更新学习率

                # 更新进度条 - 只在主进程（只在参数更新时更新）
                if opt.rank == 0:
            ...
            # 梯度累积外：验证和保存逻辑（每个批次都检查）
            # save checkpoint
            if current_iter % config['training']['save_freq'] == 0 and opt.rank == 0:
                # 保存Adapter模型
                save_filename = f'model_ad_{current_iter}.pth'
                save_path = os.path.join(experiments_root, 'models', save_filename)
                # 获取模型参数（从DDP模型中提取）
                state_dict = model_ad.module.state_dict()
                save_dict = {}
                ...
                save_path = os.path.join(experiments_root, 'models', save_filename)
                state_dict = control_injectors.module.state_dict()
                save_dict = {}
                for key, param in state_dict.items():
                    save_dict[key] = param.cpu()
                torch.save(save_dict, save_path)
            ...
            # val - 只在主进程进行验证
            if current_iter % opt.val_iter == 0 and opt.rank == 0:
                    gen_image_count+=1
                    ...
                    with torch.no_grad():
                        ...
                        features_adapter = model_ad.module(depth_data)  # 使用module访问原始模型
                        ...
                        samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                            ...
                                                            control_injectors=control_injectors.module)
                        x_samples_ddim = model.decode_first_stage(samples_ddim)
                        x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
                        x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
                        ...
                        for id_sample, x_sample in enumerate(x_samples_ddim):
                            ...
                            if composite_score > best_val_score:
                                ...
                                patience_counter = 0
                                best_model_state = model_ad.module.state_dict().copy()
                                best_control_injectors_state = control_injectors.module.state_dict().copy()
                                ...
                                if logger is not None:
                                    ...
                                    logger.info(f"✅ 最佳模型已保存到: model_ad_best.pth 和 model_control_injectors_best.pth")
                            else:
                                # 性能没有改善，增加早停计数器（已禁用）
                                patience_counter += 1
                                validation_type = "深度一致性" if depth_estimator is not None else "图像相似性"
                                if logger is not None:
                                    ...
                                    # logger.info(f"早停说明: 连续{patience}次验证无改善将自动停止训练，当前第{patience_counter}次") (早停已禁用)

                            # 早停检查已禁用 - 训练将继续完成所有epochs
                            ...
                            # 学习率调度：每优化器步骤调用一次（移除了ReduceLROnPlateau逻辑）
                            # WarmupCosineAnnealingLR将在每次优化器步骤后调用
            ...
            # 早停检查已禁用 - 训练将继续完成所有epochs
            # if opt.rank == 0 and early_stop:
            #     break

        # 记录当前epoch结束时的学习率 - 只在主进程
        if opt.rank == 0:
            current_lr = optimizer.param_groups[0]['lr']
    ...
    # 早停后模型恢复逻辑已禁用
    # if opt.rank == 0 and early_stop and best_model_state is not None:
    #     model_ad.module.load_state_dict(best_model_state)
